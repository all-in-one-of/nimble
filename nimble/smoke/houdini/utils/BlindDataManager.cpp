...
{
	const GA_ROAttributeRef attrRef = gdp->findAttribute(GA_ATTRIB_DETAIL,
			GA_SCOPE_PUBLIC, "SimData");
	const GA_Attribute *attr = attrRef.getAttribute();
	const GA_AIFBlindData *blindData = attr->getAIFBlindData();
	return blindData->getValue<smoke::core::SimData*>(attr, 0);
}

void BlindDataManager::insertSimDataPtr(GU_Detail* gdp,
		smoke::core::SimData* simDataPtr)
{
	GA_RWAttributeRef attrRef = gdp->createAttribute(GA_ATTRIB_DETAIL,
			GA_SCOPE_PUBLIC, "SimData",
			NULL, NULL, "blinddata");
